	...
	if(bordercolor){
		...
		for(j=0; j<pkl->height; j++){
			for(i=0; i<pkl->width; i++){
				hit=0;
				current = id[j*pkl->width+i];
				if(i-1>=0         && current!=id[j*pkl->width+i-1]) hit++;
				if(i+1<pkl->width && current!=id[j*pkl->width+i+1]) hit++;
				if(j-1>=0          && current!=id[(j-1)*pkl->width+i]) hit++;
				if(j+1<pkl->height && current!=id[(j+1)*pkl->width+i]) hit++;
				//if(hit)
				//	memcpy(&pkl->image[(j*pkl->width+i)*pkl->channel], bordercolor->color, pkl->channel);
				if(hit){
					for(k=0; k<pkl->channel; k++){
						pkl->image[(j*pkl->width+i)*pkl->channel+k]
							= bordercolor->color[k] * hit/4 + pkl->image[(j*pkl->width+i)*pkl->channel+k] * (4-hit)/4;
					}
				}
			}
		}
...
PKLExport int pkl_voronoi_count(PKLImage pkl, int count, PKLColor bordercolor, int test)
{
	...
	if(bordercolor){
		...
		for(j=0; j<pkl->height; j++){
			for(i=0; i<pkl->width; i++){
				hit=0;
				current = ip[j*pkl->width+i];
				if(i-1>=0         && current!=ip[j*pkl->width+i-1]) hit++;
				if(i+1<pkl->width && current!=ip[j*pkl->width+i+1]) hit++;
				if(j-1>=0          && current!=ip[(j-1)*pkl->width+i]) hit++;
				if(j+1<pkl->height && current!=ip[(j+1)*pkl->width+i]) hit++;
				//if(hit)
				//	memcpy(&pkl->image[(j*pkl->width+i)*pkl->channel], bordercolor->color, pkl->channel);
				if(hit){
					for(k=0; k<pkl->channel; k++){
