...
PKLExport int pkl_vignette(PKLImage pkl, PKLColor color, int trans, int radius, int x, int y)
{
	...
	for(i=0; i<pkl->channel; i++){
		...
		for(j=0; j<PKL_COLOR; j++){
			int d = (double)j * (t+t1 * w);
			at[i][j] = PKL_COLOR_CLIP(d);
		}
	}
	...
	for(i=0; i<pkl->height; i++){
		for(j=0; j<pkl->width; j++){
			...
			if(r > radius){
				...
				for(k=0; k<pkl->channel; k++){
					int d = (1.0-a) * at[k][*(p+k)] + a * *(p+k);
					*(p+k) = PKL_COLOR_CLIP(d);
				}
			}
		}
	}
	...
	for(i=0; i<pkl->height; i++){
		for(j=0; j<pkl->width; j++){
			...
			if(r > radius){
				...
				a = 1.0 < tr ? 1.0 : tr;
				for(k=0; k<pkl->channel; k++){
					int d = (1.0-a) * at[k][pkl->image[(i*pkl->width+j)*pkl->channel+k]] + a * pkl->image[(i*pkl->width+j)*pkl->channel+k];
					wrk[(i*pkl->width+j)*pkl->channel+k] = PKL_COLOR_CLIP(d);
				}
			}
		}
