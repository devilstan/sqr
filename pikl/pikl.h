...
#define _PIKL_
	...
	#define XMD_H
	#define PKL_DEPRECATED
#else
	#define PKLExport
	#ifdef __GNUC__
		#define PKL_DEPRECATED __attribute__((__deprecated__))
	#endif
#endif
...
//#define PKL_VERSION "pikl 3.1.0"
#define PIKL_VERSION_MAJOR 3
#define PIKL_VERSION_MINOR 1
#define PIKL_VERSION_MICRO 0

#define pikl_version() ( \
	  (PIKL_VERSION_MAJOR * 10000) \
	+ (PIKL_VERSION_MINOR *   100) \
	+ (PIKL_VERSION_MICRO *     1))
...
//モザイクを円で表現
//	radius: 半径(>=3)
//	offset: 0以外の時、ランダムな位置に円を描く
PKLExport int pkl_circle(PKLImage pkl, int radius, int offset);
...
//---------------------------------------------------------------------------------------
// カスタムテーブルを設定できるエッジ(輪郭)強調
//	matrix: マスク配列
//	row: matrixの1行の要素数
//	col: matrixの行数
//---------------------------------------------------------------------------------------
PKLExport int pkl_focus2(PKLImage pkl, int *matrix, int row, int col);
...
//---------------------------------------------------------------------------------------
// 正弦フィルター2
//	pkl_sinusoidalを2回施すことで、不思議な感じになった！
//---------------------------------------------------------------------------------------
PKLExport int pkl_sinusoidal2(PKLImage pkl, double weight, PKL_SAMPLE sample);

//---------------------------------------------------------------------------------------
// 織物風
//	テクスチャをかけたような風合いになる
//---------------------------------------------------------------------------------------
PKLExport int pkl_fabric(PKLImage pkl, double weight, PKL_SAMPLE sample);
...
PKLExport int pkl_spotmosaic(PKLImage pkl, int sx, int sy, int width, int height, int weight, PKL_SPOT type);
...
//	dotscolor: ドットの色(NULLの時はゾーンの平均色)
//	backcolor: 背景色(NULLの時は白)
//---------------------------------------------------------------------------------------
...
// r          :半径
// wavelength :波紋長(30..)
// amplitude  :振幅(0..2.0程度が美しい)
// phase      :重み(10.0)
//---------------------------------------------------------------------------------------
PKLExport int pkl_water(PKLImage pkl, int centerx, int centery, int r, double wavelength, double amplitude, double phase, PKL_SAMPLE sample);
...
//---------------------------------------------------------------------------------------
// アナグリフ加工
//	赤青めがねを使って立体に見える画像にする
//		left: 左目用画像
//		right: 右目用画像(NULLの場合,leftからアナグリフ処理をする)
//		xpos: offset-x
//		ypos: offset-y
//		is_gray: trueの時、グレーイメージのアナグリフを作成する
//		is_trim: trueの時、offsetした位置を切り取る
//
//		* RGBイメージのみ対応
//		* leftとrightイメージのサイズチェックはしない
//---------------------------------------------------------------------------------------
...
//1次元パーリンノイズ
PKLExport int pkl_perlin1d(PKLImage pkl,
	double xlen, double ylen, double xweight, double yweight, PKLColor backcolor, PKL_SAMPLE sample);

//2次元パーリンノイズ
PKLExport int pkl_perlin2d(PKLImage pkl,
	double xlen, double ylen, double xweight, double yweight, PKLColor backcolor, PKL_SAMPLE sample);

//3次元パーリンノイズ
PKLExport int pkl_perlin3d(PKLImage pkl,
	double xlen, double ylen, double xweight, double yweight, PKLColor backcolor, PKL_SAMPLE sample);
...
//	rh:    縦方向の半径
//	color:  色
//	width:  塗りナシ円の枠の太さ（この値が有効な時塗りナシになり、aaの設定も無効になる。0を設定すると無効になる。）
//	aa:    アンチエイリアスレベル(0でアンチエイリアスなし)
PKLExport int draw_ellipse(PKLImage pkl, int x, int y, int rw, int rh, PKLColor color, int width, int aa);

//線分の描画
//	aa: TRUEの時アンチエイリアスを有効にする
//	edge: TRUEの時エッジ処理をする。ただし、aa=TRUEかつwidth>=3の時のみ有効
