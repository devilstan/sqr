...
PKLExport int pkl_hydrangea(PKLImage pkl, int zone, double low, double high, double coeff, double mil, double env)
{
	...
	//カラーエンボス
	if(pkl_rock(pkl, mil, env)) return (1);
...
PKLExport int pkl_sinusoidal2(PKLImage pkl, double weight, PKL_SAMPLE sample)
{
	...
	result += pkl_sinusoidal(pkl, weight, sample);
	result += pkl_sinusoidal(pkl, weight, sample);
...
PKLExport int pkl_fabric(PKLImage pkl, double weight, PKL_SAMPLE sample)
{
	...
	result += pkl_sinusoidal(pkl, weight, sample);
	result += pkl_focus(pkl, PKL_FOCUS_BUMP);
...
PKLExport int pkl_spotmosaic(PKLImage pkl, int sx, int sy, int width, int height, int weight, PKL_SPOT type)
{
	...
	if(pkl_trim(dp, sx, sy, width, height)){
		pkl_close(dp);
		return(1);
	}
	...
	switch(type){
		case PKL_SPOT_BLUR:
			pkl_blur(dp, weight);
			break;
		case PKL_SPOT_MOSAIC:
		default:
			pkl_mosaic(dp, weight, weight);
	}

	pkl_composite(pkl, dp, sx, sy, NULL);
	pkl_close(dp);
