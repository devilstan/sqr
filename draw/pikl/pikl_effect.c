...
PKLExport int pkl_sepia(PKLImage pkl, double red_weight, double green_weight, double blue_weight)
{
	...
	r = red_weight < 0.0 ? 0.0 : red_weight > 1.0 ? 1.0 : red_weight;
	g = green_weight < 0.0 ? 0.0 : green_weight > 1.0 ? 1.0 : green_weight;
	b = blue_weight < 0.0 ? 0.0 : blue_weight > 1.0 ? 1.0 : blue_weight;
	...
	for(i=0; i<pkl->height; i++){
		for(j=0; j<pkl->width; j++){
			...
			wrk[p+0] = pkl->image[p+0] * r;
			wrk[p+1] = pkl->image[p+1] * g;
			wrk[p+1] = pkl->image[p+2] * b;
		}
	}
...
PKLExport int pkl_vtr(PKLImage pkl, int colspace, int gst, int cst)
{
	...
	for(i=0; i<pkl->height; i++){
		...
		for(j=0; j<pkl->width; j++){
			...
			for(k=0; k<pkl->channel; k++){
				int d = (pkl->image[(i*pkl->width+j)*pkl->channel+k] + pkl->image[(i*pkl->width+t)*pkl->channel+k])/2 + (cst-127)*h;
...
PKLExport int pkl_dither(PKLImage pkl, PKL_DITHER dither)
{
	...
	double *pt, err;
	int i, j, k, gray, s, t, tx, ty, row, col, xrange, d;
		...
		for(j=0; j<pkl->width; j++){
			...
			for(s=0; s<col; s++){
				for(t=-xrange; t<=xrange; t++){
					...
					d = pkl->image[(ty*pkl->width+tx)*pkl->channel] + err * pt[s*row+t+xrange];
					pkl->image[(ty*pkl->width+tx)*pkl->channel] = PKL_COLOR_CLIP(d);
				}
			}
		}
	}
	...
	for(i=0; i<pkl->height; i++){
		for(j=0; j<pkl->width; j++){
			...
			if(pix<127){
				...
				for(k=0; k<pkl->channel; k++){
					d = pkl->image[(i*pkl->width+j)*pkl->channel+k]+weight;
					wrk[(i*pkl->width+j)*pkl->channel+k] = PKL_COLOR_CLIP(d);
				}
			}
			err = pix>127 ? pix-255 : pix;
			...
			for(s=0; s<col; s++){
				for(t=-xrange; t<=xrange; t++){
					...
					if(tx<0 || tx>=pkl->width || ty>=pkl->height) continue;
					if (pt[s*row+t+xrange] < 0) continue;
					d = graypic[ty*pkl->width+tx] + err * pt[s*row+t+xrange];
					graypic[ty*pkl->width+tx] = PKL_COLOR_CLIP(d);
				}
			}
		}
	}
...
PKLExport int pkl_focus(PKLImage pkl, PKL_FOCUS type)
{
	...
	return pkl_focus2(pkl, matrix, 3, 3);
}
