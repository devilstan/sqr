...
PKLExport int pkl_mosaic(PKLImage pkl, int msx, int msy)
{
	...
	mpx = (msx<1) ? 1 : (msx>pkl->width) ? pkl->width : msx;
	mpy = (msy<1) ? 1 : (msy>pkl->height) ? pkl->height : msy;
	mlx = (pkl->width+mpx-1)/mpx;
	mly = (pkl->height+mpy-1)/mpy;
...
PKLExport int pkl_circle(PKLImage pkl, int radius, int offset)
{
	...
	for(i=0; i<=mly; i++){
		for(j=0; j<=mlx; j++){
			...
			if(offset){
				sx += rand() % (offset*2 + 1) - offset;
				sy += rand() % (offset*2 + 1) - offset;
			}
			draw_ellipse(pkl, sx+radius, sy+radius, radius, radius, &color, 0, 0);
		}
	}
